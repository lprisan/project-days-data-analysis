...
renderPlot({
  ...
  if(nrow(data)>1){ # at least, two points to plot
    ...
    ggplot(data, aes(x=timeofday, y=engagement, color=group))+
      geom_smooth(se=F)+theme_minimal()+geom_point(alpha=0.2)+ggtitle("Engagement")
...
renderPlot({
  ...
  if(nrow(data)>1){ # at least, two points to plot
    ...
    ggplot(bygroup.ineq, aes(x=timeofday, y=engagement2, color=group))+theme_minimal()+
        geom_smooth(se=F)+ggtitle("Inequality")+geom_point(alpha=0.2)+ylim(0,1)
