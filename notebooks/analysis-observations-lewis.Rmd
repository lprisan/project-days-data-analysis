...
We can build a sparse autoencoder using the function `build_sparse_autoencoder(data)` and proceeding in the same
way as above otherwise
...
```
If the model is a sparse autoencoder, the `sparse` variable shoulde be set `TRUE` in order to
...
`agg_data <- merge_and_aggregate(matched_data, iq_data, fq_data, sparseAE = TRUE)`.
