...
-----> LUIS: why not? how long does it take to run? 

```{r}
par(mfrow=c(2,1))
plot(nstates, AIC, main="Minimise (Cirlces = AIC, Diamonds = BIC)")
points(nstates, BIC, pch = 23)
plot(nstates, logLik, main="Maximise", pch = 22)
```
...
```{r, include = F}
cat1 <- subset(new_data, HMMPredS ==1)[,c("comments, disengaged", "looking", "talking", "technology", "resources", "external")]
cat2 <- subset(new_data, HMMPredS ==2)[,c("comments, disengaged", "looking", "talking", "technology", "resources", "external")]
cat3 <- subset(new_data, HMMPredS ==3)[,c("comments", "disengaged", "looking", "talking", "technology", "resources", "external")]
```
```{r, include = T}
summary(cat1)
summary(cat2)
...
```{r, include = T}
cor(new_data[,c("MCAdim1","MCAdim2","MCAdim3","HMMPredS","HMMS1","HMMS2","HMMS3")],
