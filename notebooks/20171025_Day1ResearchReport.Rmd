...
datefixer <- function(x, what){
  library(lubridate)
  olddate <- x[what]
  hours <- hour(olddate)-3 # Adjustment due to timezones?
  minutes <- minute(olddate)
  newdate <- ymd_hms(paste("2017-10-11 ",hours,":",minutes,":00",sep=""))
  newdate
}
tim_data$new.start <- as.POSIXct(apply(tim_data, 1, datefixer, "start"), origin="1970-01-01")
tim_data$new.end <- as.POSIXct(apply(tim_data, 1, datefixer, "end"), origin="1970-01-01")
...
... or how the inside/outside focus also varied over time
...
# TODO: add segment with the activities and durations for each group?

ggplot(filteredobs, aes(x=timestamp, y=focusext, col=group))+geom_smooth(span = 0.15, se=F)+theme_minimal()#+geom_point(alpha=0.1,size=4)
```
