...
This visualization is quite complex, but we can already see that different students have different kinds of engagement: some are more prone to talking (long green bars), some to manipulating reources (long blue bars). We can also do a similar profiling by group: 
...
Above, we can see for example that group 2 has a high profile of participation, quite balanced between external and intra-group focus of attention.

We can also see, for example, how the engagement varied over time (which we can relate to the different activities):

```{r pca-time}
ggplot(student_obs, aes(x=timestamp, y=participation))+geom_smooth(span = 0.15, se=F)+theme_minimal()#+geom_point(alpha=0.1,size=4)


#ggplot(student_obs, aes(x=timestamp, y=focusext))+geom_smooth(span = 0.15, se=F)+theme_minimal()#+geom_point(alpha=0.1,size=4)
```

### Engagement over time/activities

TODO: needs estimation of when each group was doing which activity
...
<!-- Not very useful, they are quite the same...
* Were there differences between the self-reported participation levels in the different activities?-->

```{r gini-act}

# aggini <- aggregate(contribution~activity, data=fb1_data, FUN=ineq)
# names(aggini)[[2]] <- "Gini"
# 
# kable(aggini)
...
We can also try to relate this equality with the learning outcomes... TODO!
...
Indeed, we can explore further this relationship between difficulty of the activity and student satisfaction with that activity:

```{r difficulty-satisfaction}
ggplot(fb1_data, aes(x=satisfaction, y=difficulty, col=activity))+geom_jitter(size=3, width=0.3, height=0.3, alpha=0.5)+theme_minimal()+scale_y_continuous(breaks=seq(0,9,1), limits=c(0,9))+scale_x_continuous(breaks=seq(0,9,1), limits=c(0,9))+geom_smooth(method="lm",se=F)#+geom_smooth(col="grey", se=F)
